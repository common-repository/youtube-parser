...
add_action( 'wp_enqueue_scripts', 'ytp_styles_register' );

function ytp_styles_register() {
	wp_register_style( 'preparserstyle', plugins_url( 'css/style.css', __FILE__ ) );
}
...
function ytp_parser( $args ) {
	wp_enqueue_style( 'preparserstyle' );
	...
	if ( isset( $uriRaw ) && ! empty( $uriRaw ) ) {
		$previewVideoStatic = new ytp_previewParserObject( $uriRaw, true );
	} else {
		$previewVideoStatic = new ytp_previewParserObject( $args['uridef'] );
	}
	if ( $width ) {
		?>
		<style>
	...
	<div class="preparser-header pp-row pp-h-width">
		<div class="pp-width">
			<h1 class="preparser-title"><?php _e( 'YouTube Preview', 'previewparser' ); ?></h1>
			<div
				class="preparser-desc"><?php echo __( 'Get an available preview of the image for your video on Youtube', 'previewparser' ); ?></div>
		</div>
		<form method="POST" class="preparser-form pp-col w8-12 off2" id="preparser-form" name="preparser-form">
			<input class="preparser-form-input" type="text" data-def="<?php echo $previewVideoStatic->id; ?>"
	...
	<div class="pp-row preparser-result-wr pp-width">
		<div class="pp-col w8-12 off2 preparser-result">
			<div class="js-result-small-wr">
				<div
					class="preparser-img-desc"><?php _e( 'Thumbnail size 120&times;90', 'previewparser' ); ?></div>
			...
			<div class="js-result-medium-wr pp-row">
				<div class="preparser-img-desc"><?php _e( 'Normal size 480&times;360', 'previewparser' ); ?></div>
			...
			<div class="js-result-full-wr pp-row">
				<div class="preparser-img-desc"><?php _e( 'Full size 1920&times;1080', 'previewparser' ); ?></div>
			...
			<div class="preparser-result-iframe preparser-result-container js-result-iframe">
				<iframe width="640" height="360" src="//www.youtube.com/embed/<?php echo $previewVideoStatic->id; ?>"
...
function ytp_parserCallback() {
	$previewVideoAjax = new ytp_previewParserObject( $_POST['url'], true );
